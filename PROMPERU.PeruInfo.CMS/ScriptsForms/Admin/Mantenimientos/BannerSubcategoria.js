$(document).ready(function () {
    ...
    $("#fileImagenDesktop").change(function (e) {
        ...
        $.ajax({
            url: `${baseURL}bannersubcategoria/upload`,
            type: "post",
            contentType: false,
            processData: false,
            data: formData,
    ...
    $("#ddlSubcategoria").change(function() {
        $("#hdnSubcategoriaId").val($("#ddlSubcategoria").val());
    });

    $("#ddlSubcategoria").trigger("change");
});
...
function ListarBloques() {
    ...
    var table = $("#gridBloques").DataTable({
        ...
        "ajax": `${baseURL}bannersubcategoria/listar`,
        "deferRender": true,
        "columns": [
            { "data": "TraduccionId" },
            { "data": "IdiomaNombre" },
        ...
        "createdRow": function (row, data, index) {
            data.Activo === true ? $("td", row).eq(3).html("Si") : $("td", row).eq(3).html("No");
        }
    });
...
function LlenarDatosBloque() {
    ...
    const subcategoriaId = $("#hdnSubcategoriaId").val();
...
function RegistrarBloque() {
    ...
    console.log(nuevaBloque);

    $.ajax({
        url: `${baseURL}bannersubcategoria/insertar`,
...
function SeleccionarBloque(bannerTraduccionId) {
    ...
    $.ajax({
        url: `${baseURL}bannersubcategoria/seleccionar`,
...
function SeleccionarBloquePorIdioma(bannerId, idiomaId) {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response !== null && response !== undefined) {
                $("#hdnSubcategoriaId").val(response.SubcategoriaId);
                $("#ddlSubcategoria").val(response.SubcategoriaId);
                $("#ddlIdioma").val(response.CategoriaId);
                $("#hdnBannerId").val(response.Id);
...
function ActualizarBloque(bannerTraduccionId) {
    ...
    $.ajax({
        url: `${baseURL}bannersubcategoria/actualizar`,
    ...
    confirm("¿Está seguro de eliminar el Bloque?",
        function (result) {
            if (result) {
                ...
                $.ajax({
                    url: `${baseURL}bannersubcategoria/eliminar`,
