$(document).ready(function () {
    ...
    $(document).keyup(function (event) {
        if (event.which === 27) {
            ...
            $("#txtFechaAvisoPrivacidad").datetimepicker("hide");
        }
    });
...
function LimpiarFormAvisoPrivacidad() {
...
function SeleccionarBloque(idiomaId) {
    ...
    $.ajax({
        url: `${baseURL}AvisoPrivacidad/SeleccionarPorIdioma`,
...
function ActualizarBloque(traduccionId) {
    ...
    $.ajax({
        url: `${baseURL}AvisoPrivacidad/actualizar`,
        ...
        success: function (response) {
            if (response) {
                ...
                alert("No se pudo actualizar el Aviso de Privacidad", "WARNING");
                return;
            }
        },
        complete: function () {
        ...
        error: function (error) {
            alert("No se pudo actualizar el Aviso de Privacidad", "ERROR");
            HideLoader();
        }
    });
}